...
function SalesManagement() {
  ...
  const fetchDailySales = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/sales?daily=true');
        ...
        <tbody>
          {filteredSales.map((sale) => (
            <tr key={sale.id}>
              ...
              <td>{sale.cliente ? `${sale.cliente.nombre} ${sale.cliente.apellido}` : 'No registrado'}</td>
              <td>
                <button
          ...
          <div className="modal-dialog modal-lg">
            <div className="modal-content">
              ...
              <div className="modal-body">
                ...
                <p><strong>Monto Total:</strong> ${selectedSale.total}</p>
                <p><strong>Medio de Pago:</strong> {paymentMethods[selectedSale.metodo_pago_id] || 'Desconocido'}</p>
                <p><strong>Cliente:</strong> {selectedSale.cliente ? `${selectedSale.cliente.nombre} ${selectedSale.cliente.apellido}` : 'No registrado'}</p>
